...
namespace vuk {
	...
	inline Value<Buffer<T>> copy(Value<ImageAttachment> src, Value<Buffer<T>> dst, VUK_CALLSTACK) {
		auto image2buf =
		    make_pass("copy image to buffer", [](CommandBuffer& cbuf, VUK_IA(Access::eTransferRead) src, VUK_ARG(Buffer<T>, Access::eTransferWrite) dst) {
			    BufferImageCopy bc;
			    bc.imageOffset = { 0, 0, 0 };
			    bc.bufferRowLength = 0;
			    bc.bufferImageHeight = 0;
	...
	inline Value<Buffer<T>> copy(Value<Buffer<T>> src, Value<Buffer<T>> dst, VUK_CALLSTACK) {
		auto buf2buf =
		    vuk::make_pass("copy buffer to buffer",
		                   [](vuk::CommandBuffer& command_buffer, VUK_ARG(Buffer<T>, vuk::eTransferRead) src, VUK_ARG(Buffer<T>, vuk::eTransferWrite) dst) {
	...
	inline Value<ImageAttachment> copy(Value<Buffer<T>> src, Value<ImageAttachment> dst, VUK_CALLSTACK) {
		auto buf2img =
		    make_pass("copy buffer to image", [](CommandBuffer& cbuf, VUK_ARG(Buffer<T>, Access::eTransferRead) src, VUK_IA(Access::eTransferWrite) dst) {
			    BufferImageCopy bc;
			    bc.imageOffset = { 0, 0, 0 };
			    bc.bufferRowLength = 0;
			    bc.bufferImageHeight = 0;
